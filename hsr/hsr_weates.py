...
no_esta = []
for i in targets:
    if i not in model_glove:
        no_esta.append(i)

for i in no_esta:
    targets.remove(i)
    
alpha = [0,10,30,60,80,100]
...
for index,fold in outer_folds:
    ...
    for param in alpha:
        ...
        model=model_glove, definitional_words=inner_train_f+inner_train_m, alpha=param)
        
        debiased_model = hsr.transform(model = model_glove,target=targets, ignore=gender_specific)
        ...
        df = pd.DataFrame(pd.DataFrame([['WEAT', weat,param], ['WEATES', weates,param],
                                       ['ECT', ect,param], ['RNSB', rnsb,param],
                                        ...
                                        columns=['Metric','Value','alpha']))
        df2 = pd.DataFrame({'Weat':weat,'Weates':weates,'RND':rnd,'RIPA':ripa,'ECT':ect, 'RNSB':rnsb, 'alpha':param},index=[index])
        all_df = pd.concat([all_df,df])
        all_df2 = pd.concat([all_df2,df2])
        if weates < best_score:
          best_score = weates
          best_param = param
    ...
    hsr_best = HalfSiblingRegression().fit(
    model=model_glove, definitional_words=outer_train_f+outer_train_m, alpha=best_param)
        
    debiased_model_best = hsr_best.transform(model = model_glove,target=targets, ignore=gender_specific)
...
bests_df = pd.DataFrame({'Weat':weats_bests,'Weates':weates_bests,'RND':rnd_bests,'RIPA':ripa_bests,'ECT':ects_bests,'RNSB':rnsb_bests,'alpha':best_params})
    

bests_df.to_csv('./results/bests_hsr_weates.csv')
all_df.to_csv('./results/all_hsr_weates.csv')
all_df2.to_csv('./results/all_hsr_weates2.csv')
print("Results saved")