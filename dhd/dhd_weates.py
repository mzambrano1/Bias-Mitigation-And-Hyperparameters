...
print("Loading model...")
# Load the model
glove_wiki = KeyedVectors.load("../../../../ablation/original models/glove-wiki-gigaword-300.kv")
...
for index,fold in tqdm(outer_folds):
    ...
    for param in tqdm(solver):
        
        #se buscan los mejores parametros para cada fold inner
        d_pairs = make_def_pairs(inner_train_m,inner_train_f)
        
        dhd = DoubleHardDebias(pca_args = {'svd_solver': param},n_words=words).fit(
        model=model_glove, definitional_pairs=d_pairs,bias_representation=['he','she'])
        
        debiased_model = dhd.transform(model = model_glove,target=targets,ignore=gender_specific)
    ...
    dhd_best = DoubleHardDebias(pca_args = {'svd_solver': best_param},n_words=words).fit(
    model=model_glove, definitional_pairs=d_pairs,bias_representation=['he','she'])
        
    debiased_model_best = dhd_best.transform(model = model_glove,target=targets,ignore=gender_specific)
...
bests_df.to_csv('./results/bests_dhd_weates.csv')
all_df.to_csv('./results/all_dhd_weates.csv')
all_df2.to_csv('./results/all_dhd_weates2.csv')
print("Results saved")